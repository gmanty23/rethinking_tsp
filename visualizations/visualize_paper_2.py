...
def plot_tour_heatmap(ax, coords, tour, cost_matrix, title, cost, gap, wind_vector, wind_mag):
    """Draws the tour with saturated green/red edges, central wind arrow, and gap."""
    ...
    for i in range(len(tour)):
        ...
        # Added path_effects to create a black outline/contour around the arrow
    ...
    ax.set_title(f"{title}\nTotal Cost: {cost:.4f}\nGap: {gap:.2f}%", fontsize=20, fontweight='bold')
    ax.axis('off')
...
def compare_models_to_lkh(coords_model_path, stats_model_path, hybrid_model_path, dataset_path, lkh_path, seed, index):
    ...
    plot_tour_heatmap(ax3, coords, hybrid_seq, cost_matrix, "Sparse Hybrid (Best Model)", matrix_hybrid_cost, hybrid_opt_gap, wind_vector, wind_mag)
    plot_tour_heatmap(ax4, coords, lkh_seq, cost_matrix, "LKH Baseline", matrix_lkh_cost, 0.00, wind_vector, wind_mag)
    
    
    out_img = f"visualizations/solution_visualization/Z_PAPER_gap_comparison_idx{target_idx}_seed{seed}.png"
    plt.tight_layout()
