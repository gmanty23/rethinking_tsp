...
# --- PATHS ---
LOG_DIR="logs_windy_tsp_none_ablation"
OUTPUT_DIR="outputs/none_ablation" # Standard base output directory for Rethinking TSP
mkdir -p $LOG_DIR
...
# Master Log File
LOG_FILE="${LOG_DIR}/tsp_none_ablation_study.log"

# Initialize Log
echo "==================================================" > "$LOG_FILE"
echo "STARTING CONTINUOUS PARALLEL NONE ABLATION STUDY" >> "$LOG_FILE"
...
for GRAPH_SIZE in "${GRAPH_SIZES[@]}"; do
    ...
    for FEAT in "${FEATURES[@]}"; do            
        RUN_NAME="none_tsp${GRAPH_SIZE}_${MODE}_${FEAT}_ent${ENTROPY}"
        ...
        echo " -> Launching: Size=${GRAPH_SIZE} | Feature=${FEAT} | Mode=${MODE}" | tee -a "$LOG_FILE"
