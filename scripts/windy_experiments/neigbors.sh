...
NEIGHBORS_VALUES=(0.75 0.5 0.35 0.3 0.25 0.2 0.15 0.1 0.05)
...
# Master Log File
LOG_FILE="${LOG_DIR}/tsp50_knn_neighbor_study.log"

# Initialize Log
echo "==================================================" > "$LOG_FILE"
echo "STARTING CONTINUOUS PARALLEL KNN NEIGHBOR STUDY" >> "$LOG_FILE"
...
for STRAT in "${KNN_STRATS[@]}"; do
    for NEIGHBOR in "${NEIGHBORS_VALUES[@]}"; do
        for MODE in "${MODES[@]}"; do
            ...
            # Standard output format: outputs/windy_tsp_20/RUN_NAME_20260304T120000
            ...
            # 'jobs -r -p' lists PIDs of currently running background jobs in this shell
            while [ $(jobs -r -p | wc -l) -ge $MAX_PARALLEL_JOBS ]; do
                sleep 5
            done
            
            echo " -> Launching: Strat=${STRAT} | Neighbors=${NEIGHBOR} | Mode=${MODE}" | tee -a "$LOG_FILE"
